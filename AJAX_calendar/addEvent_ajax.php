...
if(isset($date)&&isset($time)&&isset($title)&&isset($content)){
    ...
    $stmt = $mysqli->prepare("insert into events (user_id, event_date, event_time, event_title, event_content, event_group) values (?, ?, ? , ? , ? ,?)");
    if(!$stmt){
    echo json_encode(array(
		"success" => false,
    ...
    $stmt->bind_param('issssi', $user_id, $date, $time, $title, $content, $group);
